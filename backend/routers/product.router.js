...
router.post("/add",upload.array("images"),async(req,res)=>{
    response(res,async()=>{
        ...
        await product.save();
        res.json({message:"Ürün kaydı başarıyla eklendi!"});
    });
});
...
router.post("/",async(req,res)=>{
    response(res,async()=>{
        ...
        }).sort({name:1}).populate("categories").skip((pageNumber-1)* pageSize).limit(pageSize);
...
//Ürünün Active Pasif durumunu değiştir
...
router.post("/removeImageByProductIdAndIndex",async(req,res)=>{
    response(res,async()=>{
        ...
        if (product.imageUrls.length ==1) {
            res.status(500).json({message:"Son ürün resmini silmezsiniz en az bir ürün resmi bulunmaktatır!"})
        } else{
            let image = product.imageUrls[index];
            product.imageUrls.splice(index,1);
            await Product.findByIdAndUpdate(_id,product);
            fs.unlink(image.path,()=>{});
            res.json({message:"Resim başarıyla kaldırırldı!"});
        }
    })
})
